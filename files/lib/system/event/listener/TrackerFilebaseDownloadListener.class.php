...
/**
 * Listen to Filebase download page.
 * 
 * @author		2016-2022 Zaydowicz
 * @license		GNU Lesser General Public License <http://opensource.org/licenses/lgpl-license.php>
 * @package		com.uz.tracker.filebase
 */
class TrackerFilebaseDownloadListener implements IParameterizedEventListener {
	/**
	 * tracker and link
	 */
	...
	public function execute($eventObj, $className, $eventName, array &$parameters) {
		...
		if (!$this->tracker->wlfilFileDownload) return;
		
		$this->link = $eventObj->file->getLink();
		if ($eventObj->file->isCommercial) $this->store('wcf.uztracker.description.filebase.download.commercial', 'wcf.uztracker.type.wlfil');
		else $this->store('wcf.uztracker.description.filebase.download', 'wcf.uztracker.type.wlfil');
	}
