...
class TrackerFilebaseDownloadListener implements IParameterizedEventListener
{
    ...
    public function execute($eventObj, $className, $eventName, array &$parameters)
    {
        ...
        if (!$this->tracker->wlfilFileDownload) {
            return;
        }

        $this->link = $eventObj->file->getLink();
        if ($eventObj->file->isCommercial) {
            $this->store('wcf.uztracker.description.filebase.download.commercial', 'wcf.uztracker.type.wlfil');
        } else {
            $this->store('wcf.uztracker.description.filebase.download', 'wcf.uztracker.type.wlfil');
        }
    }
